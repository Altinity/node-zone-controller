...
func NewController(clientset kubernetes.Interface, labelKey string) *Controller {
...
func (c *Controller) syncHandler(ctx context.Context, key string) error {
	...
	zone, ok := node.Labels[c.labelKey]
	if !ok || zone == "" {
		return nil
	}

	if err := c.setZoneLabel(ctx, node, zone); err != nil {
...
func (c *Controller) setZoneLabel(ctx context.Context, node *corev1.Node, zone string) error {
	if _, ok := node.Labels[zoneLabel]; ok {
		slog.Debug("Zone labels already set, skipping patch", "node", node.Name, "zone", zone)
		return nil
	}
